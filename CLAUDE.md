...
- **MathView.tsx**: moraic binary lens (moving=1/quiescent=0), sequence period + distinct-rotation counts, group-theory panel (cyclic group Cₙ action, stabilizer C₍ₙ/ₚ₎ via `stabilizerOrder`, orbit–stabilizer equation, SymmetryStar performing the stabilizer ON the circle's own labeled units — ghost ring rotates by p and lands on itself («تطابُق!»), chords write as unit-colored trails of that landing, {n/p} star; circle 4 shows the failing k=1 attempt with persistent ✕ marks then C₁; unit legend + ↻ replay, cross-circle symmetry table), full rotation table per circle (16 used + 5 neglected = 21 distinct rotations)
...
- **ExploreView.tsx** + **components/explore/**: «شكل القصيدة» setosa-style explorables sharing one circle selector — UnrollingCircle (circle↔tape morph, clickable cut), OrbitCollapser (rotation strips merge into orbit piles), SymmetryMachine (drag ghost ring to discover the stabilizer; star auto-draws), DivisorSpiral (candidate-k chords + match lamp), RhythmClock (WebAudio dum/tak playback, movable cut during play); shared helpers in `explore/geometry.ts`, rotation readings via `rotationReading()` in data/rotations.ts
- **LadderView.tsx** + **data/ladder.ts**: «سُلَّم التجريد» — the bottom-up view of the system as a combinatorial construction in five rungs: the letter (two states, 1/0) → the unit (five clusters; three of them the single family 1^k·0, plus the two irregulars, with the reveal that مجموع 110 / مفروق 101 are *readings of the bit pattern* — same letters, only the sukun moves) → the foot (classical feet derived as ARRANGEMENTS of a chosen multiset of units, not tabulated) → the meter → the circle (the line closed into a necklace). Everything is computed from the unit alphabet by permutation and concatenation; `LADDER_COUNTS` is the funnel 2→5→10→8→16→5. The pedagogical twist lives at rung 3: exactly two arrangements collide (مستفعلن = مستفع لن and فاعلاتن = فاع لاتن), both dragged in by the split peg — so abstraction first loses information two rungs *below* the circles, and that loss is precisely why دائرة المشتبه is "the confusable" (ties directly to TadakhulView). Tested in `data/ladder.test.ts`, including the collision census and that the alphabet covers every unit the circles actually use.
- **TadakhulView.tsx** + **data/tadakhul.ts**: «خريطة التداخل» — the confusion map. The circles record exact sameness (rotation); this view records audible sameness (ziḥāf, lossy). Animated bridge scene showing the fāṣila→two-cords collapse mapping circle 2's necklace onto circle 3's (tested: `applyRewrite(c2)===c3`), explaining كامل→رجز (iḍmār) and وافر→هزج (ʿaṣb) as one morphism and الرمل as the reading born in the split; deterministic SVG graph (5 circle columns, edges typed collapse/lossy/rotation, الرجز as the sink, 8 untouched meters dashed); per-edge anatomy panel (foot before/after as unit chips or bare moraic strings where ziḥāf breaks unit grammar, adjudication rule يُحمل المشترك على الأصل, disputed شواهد). Graph animates on mount (nodes pop, edges Write, selected edge streams dashes toward its target via `.animate-tad-flow`); each anatomy panel carries «المِسمَع» the listening bench (SoundBench: WebAudio loop via shared `utils/percussion.ts` hit(), live ziḥāf toggle silences the ringed letter mid-play, rotation edge gets an anchor-rest toggle making phase ambiguity audible). Thesis: rotation never confuses, only loss confuses — except خفيف/مجتث, rotation-kin blurred when الجزء shortens the anchor
